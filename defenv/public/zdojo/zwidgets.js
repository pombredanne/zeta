...
[ dijit._Widget, dijit._Templated, dijit._KeyNavContainer ],
{
  ...
  _refreshattachs : function( fetch ) {
      ...
      for ( id in newattachs ) {
          ...
          close.innerHTML = this.editable ? '&times; ' : '&ensp; ';
...
[ dijit._Widget, dijit._Templated, dijit._KeyNavContainer ],
{
  ...
  _style: { position: "absolute", border: "2px solid green",
...
tt_template = '\
  <div class="ml50 mr50" style="display: none;">\
      <div class="bgyellow1 pb5" \
...
[ dijit._Widget, dijit._Templated, dijit._KeyNavContainer ],
{
  ...
  postCreate: function() {
      ...
      if( tt ) {
          ...
          if( this.n_tooltip ) {
              dojo.connect(
                this.n_tooltip, 'onclick',
                this,
                function(e) {
                    dojo.style( this.domNode, 'display' ) == 'none' ?
                        dojo.fx.wipeIn({ node : this.domNode, duration : 300 }).play()
                        : dojo.fx.wipeOut({ node : this.domNode, duration : 300 }).play();
...
zrc_template = '\
<div class="rcomments mb5" \
   style="border-top: 2px solid gray; border-left: 1px solid gray; border-right: 1px solid gray">\
  <table class="w100">\
  <tr class="pl10 bggray1 hdr pt5" style="height: 2em;">\
      <td class="commentor fntsmall p2" style=""></td>\
      <td class="nature pt2 pb2 pl10 pr10" style="width: 6em;"></td>\
      <td class="action pt2 pb2 pl10 pr10" style="width: 6em;"></td>\
      <td class="approved pt2 pb2 pl10 pr10" style="width: 6em;"></td>\
      <td class="reply fgblue pointer pt2 pb2 pl10 pr10" style="width: 4em;"></td>\
  </tr>\
  <tr class="comment pl10"><td colspan="6" class=" p5">\
...
[ dijit._Widget, dijit._Templated, dijit._KeyNavContainer ],
{
  ...
  _make_commentnode: function() {
      ...
      var n_trs        = dojo.query( 'tr', this.domNode.childNodes[1] );
      var n_hdr        = n_trs[0];
      var n_comment    = n_trs[1].childNodes[0];
      ...
      dojo.create( 'a',{ href  : commentbyurl,
                         ...
                         innerHTML: commentby },
                         hdr_children[1], 'last' );
      dojo.create( 'span', { class : 'ml20 fntitalic', innerHTML : 'on'+datestr },
                   hdr_children[1], 'last' );

      // nature
      if( w_cntnr.revwcmtable ) {        // Nature, participants, moderator and author can change
          dojo.place( w_cntnr.make_selnature(), hdr_children[3], 'last' );
          if( nature ) {                 // Nature
              opt = dojo.query( 'option[value='+nature+']', hdr_children[3] )[0]
              if( opt ) {
                  dojo.attr( opt, 'selected', 'selected' )
              }
          } else {
              opts = dojo.query( 'option', hdr_children[3] );
              if( opts ) {
                  dojo.attr( opts[0], 'selected', 'selected' )
              }
          }
      } else {
          hdr_children[3].innerHTML = nature ? nature : '<em>no nature</em>'
          dojo.style( hdr_children[3], { textAlign: 'center' });
      }

      // action
      if( w_cntnr.authored ) {        // Action, only the author can change
          dojo.place( w_cntnr.make_selaction(), hdr_children[5], 'last' );
          if( action ) {
          ...
          dojo.style( hdr_children[5], { textAlign: 'center' });
      }
      // approve
      if( w_cntnr.moderated ) {           // Approve, only the moderator can approve
          checked = approved ? 'checked="checked"' : '';
          hdr_children[7].innerHTML = 'Approve: <input type="checkbox" '+checked+'"></input>'
      } else {
          hdr_children[7].innerHTML = approved ?
                                          '<em class="fntbold fggray">Approved</em>' 
                                          : '<em class="fntbold fgcrimson">Pending</em>'
          dojo.style( hdr_children[7], { textAlign: 'center' });
      }

      // Connect to handlers, to submit nature, action and approve changes.
      dojo.connect( dojo.query( 'select', hdr_children[3] )[0], 'onchange',
      ...
      if( w_cntnr.moderated ) {
          dojo.connect( dojo.query( 'input[type=checkbox]', hdr_children[7] )[0], 'onclick',
      ...
      if( w_cntnr.revwcmtable ) {
          hdr_children[9].innerHTML = 'Reply'
          dojo.connect( hdr_children[9], 'onclick',
...
zrcc_template = '<div class="revwcomments" style="margin: 10px auto; max-width: 800px;"></div>'
...
[ dijit._Widget, dijit._Templated, dijit._KeyNavContainer ],
{
  ...
  naturenames: null,      // Possible nature for review comment

  actionnames: null,      // Possible action than can be taken on review comment
  ...
  make_selnature : function() {
      var n_sel = dojo.create( 'select', { name: 'reviewnature' } );
      dojo.forEach(
        this.naturenames,
        function(naturename) {
            dojo.create( 'option', { value: naturename, innerHTML: naturename },
                         n_sel, 'last' );
        }
      );
      return n_sel;
  ...
  make_selaction : function() {
      var n_sel = dojo.create( 'select', { name: 'reviewaction' } );
      dojo.forEach(
        this.actionnames,
        function(actionname) {
            dojo.create( 'option', { value: actionname, innerHTML: actionname },
                         n_sel, 'last' );
        }
      );
      return n_sel;
