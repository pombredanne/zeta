...
function flashmsghandler( type, msg, timeout ) {
    ...
    if( type == 'hide' && timeout ) {
        dojo.fx.wipeOut({ node: n_divblk, delay: timeout, duration: 1000 }).play();
    } else if ( type == 'hide' ) {
        dojo.style(n_divblk, 'display', 'none');
    } else if ( type == 'error' ) {
        dojo.removeClass( n_divblk, 'bgyellow' );
        dojo.addClass( n_divblk, 'bgLSalmon' );
        if( dojo.style(n_divblk, 'display') == 'none' ) {
            n_div.innerHTML = msg;
            dojo.style(n_divblk, 'display', 'block');
        } else {
            n_div.innerHTML = n_div.innerHTML + ' ; ' + msg;
        }
    } else if ( type == 'message' ) {
        if( dojo.hasClass(n_divblk, 'bgLSalmon') && (dojo.style(n_divblk, 'display') == 'block') ) {
            n_div.innerHTML = n_div.innerHTML + ' ; ' + msg;
        } else {
            dojo.removeClass( n_divblk, 'bgLSalmon' );
            dojo.addClass( n_divblk, 'bgyellow' );
            n_div.innerHTML = msg;
            dojo.style(n_divblk, 'display', 'block');
        }
    }
...
function url_preview(url, previewtype) {
    return url + '&previewtype=' + previewtype
}
...
/*************** ZWiki Library implementing table sorter ************/
...
function tablesorter() {
    ...
    var fortable = function(n_table) {
        ...
        /* Organise table in column wise (the HTML DOMs provide them in
         * row-wise */
...
/******** Library implementing ZWiki box extension show/hide toggler  *********/

function togglezwbox() {
